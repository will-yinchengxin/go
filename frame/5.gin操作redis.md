...
可以用go env -u 恢复初始设置；GOPROXY的值应该还可以是https://mirrors.aliyun.com/goproxy/  或 https://goproxy.cn,direct
...
![](pic/3a88f804.png)
## 1.安装及使用
docker启动一个名为redis-5.0.7的5.0.7版本的redis server示例：

``docker run --name redis-5.0.7 -p 6379:6379 -d redis:5.0.7``
...
````
#### [帮助文档](https://pkg.go.dev/github.com/go-redis/redis)
